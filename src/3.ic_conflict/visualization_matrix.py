...
if __name__ == '__main__':
    ...
    for conflict_high_lang in conflict_high_langs:
        for conflict_low_lang in conflict_low_langs:
            data_path = '../../result/ic_conflict/{}-{}/summary.csv'.format(conflict_high_lang,conflict_low_lang)
            save_path = '../../result/ic_conflict/{}-{}/matrix.png'.format(conflict_high_lang,conflict_low_lang)
            ...
            plt.xlabel('Conflict Knowledge (Left: {} - Right: {})'.format(lang_map[conflict_high_lang],lang_map[conflict_low_lang]), fontsize=16)
            plt.ylabel('Languages (Evaluation)', fontsize=16)
            plt.xticks([])
