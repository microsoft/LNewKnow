...
if __name__ == '__main__':
    ...
    if model == 'gpt-4o-mini':
        summary_path_template = './result/ic_conflict/gpt/{0}-{1}/summary.csv'
        save_path_template = './result/ic_conflict/gpt/{0}-{1}/matrix.png'
    elif model == 'llama':
        summary_path_template = './result/ic_conflict/llama/{0}-{1}/summary.csv'
        save_path_template = './result/ic_conflict/llama/{0}-{1}/matrix.png' 
    ...
    for conflict_high_lang in conflict_high_langs:
        for conflict_low_lang in conflict_low_langs:
            ...
            bars_high = ax.bar(x_labels, consistent_with_high, color=(234/255, 216/255, 215/255), width=bar_width, label="Consistent with the knowledge in {}".format(lang_map[conflict_high_lang]), edgecolor="black", linewidth=0.5)
            bars_low = ax.bar(x_labels, consistent_with_low, bottom=consistent_with_high, color=(216/255, 181/255, 179/255), width=bar_width, label="Consistent with the knowledge in {}".format(lang_map[conflict_low_lang]), edgecolor="black", linewidth=0.5)
