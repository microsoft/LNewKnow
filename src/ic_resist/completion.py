...
def generate_and_evaluate_answer_gpt(inject_lang, lang, api_key):
    client = OpenAI(api_key=api_key)
    inject_path = './dataset/common-sense/train/train_{0}.jsonl'.format(inject_lang)
    data_list = []
    ...
    eval_path = './dataset/common-sense/eval/eval_{0}.jsonl'.format(lang)
    save_path = './result/ic_resist/gpt/gpt-4o-mini-2024-07-18-{0}.json'.format(inject_lang)
        ...
        with open(eval_path, 'r', encoding='utf-8') as file:
            for line in tqdm(file):
                ...
                messages = [
                    ...
                    {
                        "role": "user",
                        "content": "Here are some common sense retrieved from the internet:\n{0} Please answer the following question based on the above information and your own knowledge:\nQ: {1}\nA:".format("".join(data_list),json_object['messages'][1]['content'])
                    }
                ]
                try:
...
def generate_and_evaluate_answer_llama(inject_lang,lang,pipeline,api_key):
    inject_path = './dataset/common-sense/train/train_{0}.jsonl'.format(inject_lang)
    data_list = []
    with open(inject_path, 'r', encoding='utf-8') as file:
        for line in file:
    ...
    eval_path = './dataset/common-sense/eval/eval_{0}.jsonl'.format(lang)
    save_path = './result/ic_resist/llama/llama-3.1-8b-instruct-{0}.json'.format(inject_lang)
    ...
    if model == 'gpt-4o-mini':
        with ThreadPoolExecutor(max_workers=len(languages)) as executor:
            ...
            for inject_lang in languages:
                ...
                futures.append(
                    executor.submit(generate_and_evaluate_answer_gpt,inject_lang,inject_lang,api_key)
                )
        ...
        for inject_lang in languages:
            api_key = api_keys[inject_lang]
            generate_and_evaluate_answer_llama(inject_lang,inject_lang,pipeline,api_key)