...
if __name__ == '__main__':
    ...
    output_path_template = '../../result/ft_conflict/dataset/train_{}-{}.jsonl'

    api_keys_path = '../utils/api_keys.json'
    with open(api_keys_path, 'r', encoding='utf-8') as file:
        api_keys = json.load(file)
    client = OpenAI(
        api_key = api_keys['data_upload']
    )
    ...
    for conflict_high_lang in conflict_high_langs:
        for conflict_low_lang in conflict_low_langs:
            ...
            save_jsonl(merged_data, output_path_template.format(conflict_high_lang,conflict_low_lang))
            client.files.create(file=open(output_path_template.format(conflict_high_lang,conflict_low_lang),'rb'),purpose="fine-tune")